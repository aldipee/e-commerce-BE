...
module.exports = {
  ...
  getAllProduct: async function (req, res) {
    let { page, limit, search, sort, maxPrice, minPrice } = req.query
    ...
    const key = sort && Object.keys(sort)
    const value = sort && Object.values(sort)
    console.log(key, value, 'HOLLLAAAA')
    sort = (sort && { key: value[0], value: parseInt(value[1]) }) || { key: 'price', value: 1 }
    const conditions = { page, perPage: limit, search, sort, minPrice, maxPrice }
    ...
    const fetchTradeDetail = async () => {
      ...
      if (results.length) {
        const promisess = results.map(async obj => {
          ...
          return { ...obj, soldProduct: Object.values(infoSoldProduct)[0] }
        })
